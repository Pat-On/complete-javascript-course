...
const poll = {
  ...
  registerNewAnswer() {
    let arrayOfOption = this.options
    arrayOfOption = arrayOfOption.join("\n")

    let answer = parseInt(prompt(`${this.question}\n${arrayOfOption}\n(Write option number)`));

    // I need always to check if the input is what I want to receive!     
    // Always to check if the input has sense! ALWAYS! 
    typeof answer === 'number' && answer < this.answers.length && this.answers[answer]++; // short circuiting = VERY NICE! && example

    // this.displayResults(prompt("display as array or string?"))
    this.displayResults()
...
// His version to study: !IMPORTANT
// const poll = {
//   question: 'What is your favourite programming language?',
//   options: ['0: JavaScript', '1: Python', '2: Rust', '3: C++'],
//   // This generates [0, 0, 0, 0]. More in the next section 😃
